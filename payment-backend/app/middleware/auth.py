"""payment-backend 자체 JWT 미들웨어.

main-backend 와 동일한 JWT_SECRET 으로 cookie 를 검증한다. 따라서 frontend → payment-backend 직접
호출에서도 동일한 인증 모델이 동작한다. main-backend 와 코드 중복은 의도적 — shared 패턴
미적용 정책.
"""

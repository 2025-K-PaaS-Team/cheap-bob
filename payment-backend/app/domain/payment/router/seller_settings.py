"""seller 의 결제 설정. payment-backend 가 직접 노출 — frontend → payment-backend 직접 호출.

store_id 는 main-backend 내부 (seller_email→store_id 매핑) 를 internal API 로 조회한다.
"""
...
async def _current_seller_store_id(current_user: CurrentSellerDep) -> str:
    """seller_email → store_id — main-backend internal API 로 조회.

    main-backend 의 seller_store_read_service.get_store_id_by_seller_email 와 동일.

...
DLQ:
  - `TerminalEventError` → 즉시 `<topic>.dlq` + offset 전진.
  - 일반 Exception → in-memory retry counter, CONSUMER_MAX_ATTEMPTS 도달 시 DLQ.
"""
...
class TerminalEventError(Exception):
    """retry 가 결과를 바꾸지 못하는 영구 실패. consumer 가 즉시 DLQ 로 보낸다."""


def extract_event_id_header(msg: ConsumerRecord) -> UUID:
    """outbox-relay 발행 메시지의 event_id 헤더 추출. 누락 / 형식 오류 모두 terminal."""

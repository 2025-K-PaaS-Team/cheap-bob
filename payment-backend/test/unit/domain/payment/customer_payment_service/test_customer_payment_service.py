...
# ────────────────────────────────────────────────────────────────────
# init_payment — main-backend HTTP 으로 product/stock 조회 + cart local 생성
# ────────────────────────────────────────────────────────────────────
...
class TestConfirmPayment:
    ...
    async def test_order_create_4xx_triggers_refund(
        ...
        """main-backend 가 4xx 로 명시적 거부 → 환불 + 보상 + raise."""
        cart = CartItemFactory.create(
    ...
    async def test_order_create_5xx_does_not_refund(
        ...
        """main-backend 5xx → commit 여부 불명. 환불 금지, cart 유지, sweep 이 retry.

        regression: 5xx 시 환불해버리면 main-backend 가 실제 commit 된 주문이 외로워지는 사고.
        """
        cart = CartItemFactory.create(
            customer_id="alice@example.com", product_id="PRD_x", quantity=1,
        )
...
class TestSweepExpiredCarts:
    ...
    async def test_backend_unavailable_skips_cart(
        ...
        """is_paid 분기에서 main-backend 5xx 발생 — 다음 sweep 으로 미룸."""
        cart = CartItemFactory.create(
    ...
    async def test_paid_but_backend_4xx_triggers_refund(
        ...
        """PAID 인데 main-backend 가 명시적 거부 → 환불 + cancel."""
        cart = CartItemFactory.create(

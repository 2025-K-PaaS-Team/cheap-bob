"""payment-backend 분리 후 mocks — HTTP client + cart_item service 중심.
...
CustomerPaymentService 가 의존하는 6개:
  ...
  - internal_seller_client (HTTP → main-backend.seller)
  - internal_order_client (HTTP → main-backend.order)
"""
...
class InternalSellerClientMockFactory:
    """HTTP → main-backend.seller — find_product, get_today_operation, consume/restore_stock."""
...
class InternalOrderClientMockFactory:
    """HTTP → main-backend.order — create_order_from_cart (멱등)."""

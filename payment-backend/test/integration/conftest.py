"""payment-backend 통합 테스트 공통 설정.

실 PostgreSQL (payment-backend 전용 DB) 에 대고 서비스 → repository → DB 전체 흐름을 검증한다.

환경변수 ``POSTGRES_TEST_URL`` 이 설정돼 있어야 실행되며, 미설정 시 모든 통합 테스트는
자동 skip 한다. main-backend 와 별도 DB 인스턴스 — main-backend 의 ``POSTGRES_TEST_URL`` 과 같은
...
payment-backend 모델은 stores / sellers 등 main-backend 도메인 테이블에 FK 가 없으므로
...
async def engine():
    ...
    우선한 선택 — main-backend conftest 와 동일 정책.
    """

...
class PortOnePaymentClient:
    ...
    async def cancel_payment(
        ...
        Body 명세 (v2): https://developers.portone.io/api/rest-v2/payment#post-/payments/-paymentId-/cancel
        """
...
def _parse_payment(data: dict[str, Any], expected_payment_id: str) -> PortOnePayment:
    """v2 응답 → `PortOnePayment`. 필수 필드 누락 시 `PortOnePaymentNotFoundError`."""
    try:
        payment_id = data["id"]
...
def _extract_method(method: Any) -> Optional[str]:
    """PortOne v2 의 method 는 객체일 수 있다 — 표시 가능한 형태로 추출."""
    if method is None:
        return None

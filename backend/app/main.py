...
# uvicorn 의 cwd 가 어디든 동작하도록 /backend 만 sys.path 최상위에 둔다.
# /backend/app 을 추가하면 같은 .py 파일이 `database.session` 과 `app.database.session` 두 모듈명으로 동시에 로드돼 SQLAlchemy MetaData 가 중복 등록되는 사고가 발생한다.

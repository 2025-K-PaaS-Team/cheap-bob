...
class StoreOperationInfoRepository(BaseRepository[StoreOperationInfo]):
    ...
    async def apply_modification(
        ...
        """modification 값으로 운영 정보 업데이트 (스케줄러 worker 호출). returning rowcount."""
    ...
    async def update_today_open_status_for_stores(
        self,
        store_ids: List[str],
        day_of_week: int,
    ) -> int:
        """주어진 store_ids 에 대해 오늘 요일의 is_currently_open 을 is_open_enabled 로 동기화.
    ...
    async def get_by_day_of_week(self, day_of_week: int) -> List[StoreOperationInfo]:
        """특정 요일의 모든 운영 정보 조회 (스케줄러 worker 호출)."""
        return await self.get_many(

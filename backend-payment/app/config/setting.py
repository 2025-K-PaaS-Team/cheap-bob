...
class Settings(BaseSettings):
    """payment-svc 의 환경 설정. backend 와 별도 DB / 별도 .env 지만 JWT_SECRET 등 일부는
    환경변수로 공유한다.
    """
    ...
    # 암호화 — backend 와 동일 JWT_SECRET 을 공유 (양쪽 다 cookie 를 직접 검증).
    JWT_SECRET: str
    JWT_ALGORITHM: str
    ...
    # backend 와 동일 토큰을 양쪽에 주입한다. 양 서비스가 서로의 /api/internal/* 를 호출할 때

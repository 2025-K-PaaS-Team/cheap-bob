"""customer 의 /payment/init / /payment/confirm — payment-svc 의 핵심 오케스트레이션.

MSA 분리 후 변경점:
  - product/stock 조회 + 차감/복원은 HTTP 로 backend 호출.
  - order 생성도 HTTP 로 backend 호출 (멱등 키 = payment_id).
  - cart_items 는 payment-svc 자체 DB — FOR UPDATE / SKIP LOCKED 동시성 의미 유지.
...
원자성:
  ...
  - finalize 후처리 실패 분기 :
      4xx (OrderCreateFailedError) → backend 가 명확히 거부 → 환불 + 보상.
      5xx (BackendUnavailableError) → backend commit 여부 불명 → **환불 금지**, cart 유지,
                                       sweep 이 멱등 retry. 환불하면 backend 에 commit 된
                                       주문이 외로워지는 사고가 난다.
"""
...
class CustomerPaymentService:
    ...
    async def init_payment(
        ...
        """결제 init — backend stock 차감 + payment-svc cart 생성.

        한 트랜잭션이 아님 (다른 시스템). 순서:
          1) backend product 조회 + pickup 검증 + 재고 검증 (read)
          2) payment-svc payment_info 조회 + payment_id 생성
          3) backend consume_stock(payment_id 멱등) — write
          4) payment-svc cart_items INSERT — write
          5) (4) 실패 시 backend restore_stock(payment_id) best-effort
        """
        ...
        # backend 재고 차감 (payment_id 멱등 — 일시 장애 시 다시 호출돼도 동일 결과).
    ...
    async def _finalize_payment(self, *, payment_id: str) -> bool:
        ...
        FOR UPDATE 잠금이 PortOne 호출 + backend create_order HTTP 동안 유지되어 confirm/sweep
        ...
        실패 분기:
          ...
          - OrderCreateFailedError (4xx): backend 가 명확히 거부. PortOne 환불 + 보상.
          - BackendUnavailableError (5xx/네트워크): backend commit 여부 불명. **환불 금지**.
            cart 유지 (raise 로 outer tx 롤백 → cart delete 가 안 일어남). sweep 이 멱등 retry
            로 자동 복원. 환불 시 backend commit 된 주문이 외로워지는 사고 방지.
        """
        ...
        try:
            ...
            # 4xx — backend 가 명확히 거부 (예: validation 실패). 환불 + 보상.
            logger.exception(
            ...
            # 5xx / 네트워크 — backend commit 여부 불명. 환불하면 commit 된 주문이
    ...
    async def sweep_expired_carts(self, *, limit: int = 100) -> SweepResult:
        ...
        for cart in expired:
            try:
                ...
                result.transient += 1
                logger.warning(
                    "sweep — backend 일시 장애로 미룸 payment_id={}: {}",
                    cart.payment_id, e,
                )
                continue
            except Exception:
    ...
    async def _sweep_cart(self, cart) -> str:
        ...
        Returns:
            "finalized" — PortOne PAID 확인 → backend order 생성 (멱등).
            "cancelled" — 결제 기록 없음 / PAID 아님 / 금액 불일치 → restore + cart 삭제.
            (또는 backend 4xx 시 환불 + cancelled 로 종료.)

        Raises:
            PortOneTransientError: PortOne 5xx — caller 가 다음 sweep 으로 미룸.
            BackendUnavailableError: backend 5xx/네트워크 — caller 가 다음 sweep 으로 미룸.
        """
        ...
        if is_paid:
            # confirm 누락 → backend order 생성 (payment_id 멱등). cart 삭제.
            try:
                await self.internal_order_client.create_order_from_cart(
                    payment_id=cart.payment_id,
                ...
                # 4xx — backend 가 명확히 거부. PAID 인데 order 생성 불가 — 환불 처리.
                logger.exception(

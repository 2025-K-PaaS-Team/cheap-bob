"""payment-svc <-> backend 간 internal API 의 request/response 모델.

shared 패턴 미적용 — backend 측에도 동일한 schema 가 자체 정의로 존재한다.
계약이 바뀔 때는 양쪽을 같이 수정.
"""
...
class CreateOrderFromCartRequest(BaseModel):
    """payment-svc → backend: 결제 finalize 시 order_current_item 생성.

    payment_id 가 UNIQUE 멱등 키 — 같은 payment_id 로 두 번 호출되면 두 번째는 no-op.
    customer preference snapshot 은 backend 가 내부적으로 lookup.
    """
    payment_id: str
    product_id: str
    customer_id: str
...
class RefundRequest(BaseModel):
    ...
    payment_id: str
    store_id: str  # backend 에 secret_key 가 없으니 store_id 로 payment-svc 내부 조회
    reason: str

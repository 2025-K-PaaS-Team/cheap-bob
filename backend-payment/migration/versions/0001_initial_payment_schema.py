"""initial payment-svc schema — store_payment_info + cart_items

backend 의 동일 테이블 (마이그레이션 b8d40f1c6e1a 에서 DROP) 의 자체 DB 버전.
backend 측 stores / store_product_info 와의 FK 는 분리된 DB 이므로 제거.
...
def upgrade() -> None:
    """Upgrade — payment-svc 의 모든 테이블 CREATE."""
    op.create_table(
        "store_payment_info",

...
효과:
    ...
    스레드/커넥션이 timeout 동안 자원이 고갈되는 것을 막는다 — cascading failure 차단.

호출 측 책임:
    retry 와 함께 쌓을 때는 retry 가 바깥, circuit breaker 가 안쪽. retry 의 do_not_retry
    에 CircuitBreakerOpenError 를 넣어 "회로가 열린 상태에서 의미 없는 재시도" 를 막는다.
"""
import time
import enum
import asyncio
...
class CircuitBreaker:
    ...
    async def call(self, func, *args, **kwargs):
        """func 를 회로를 거쳐 호출. OPEN 이면 즉시 실패."""
    ...
    async def _on_failure(self) -> None:
        async with self._lock:
            # HALF_OPEN 에서 실패는 곧장 OPEN 으로 되돌린다.

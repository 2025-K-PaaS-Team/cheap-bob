...
- 예시:
  LOG_LEVEL=DEBUG
  LOG_FORMAT=console
  LOG_FILE_PATH=/main-backend/logs/app.log
...
필드 설명:
  ...
  LOG_ROTATION:
    - "100 MB" → 로그 파일이 100MB 를 넘으면 새 파일로 롤테이션
    - 용량/시간 기준 문자열 모두 사용 가능 (Loguru 규칙)
...
사용법:
  ...
  logger.bind(user_id=123).bind(user_type="customer").info("서비스 완료")
  logger.bind(user_id=123, user_type="customer").info("서비스 완료")

포맷팅 컨벤션:
  - f-string 대신 loguru 의 {} 포맷 사용
  - 로그 레벨이 비활성화된 경우 문자열 조합을 건너뛰어 불필요한 연산 방지

  사용법:
    logger.info("유저: {}", user_id)               # {}     : 범용 (str() 호출)
    logger.info("처리 시간: {:.2f}초", elapsed)     # {:.2f} : 소수점 2자리 실수
    logger.debug("입력값: {!r}", some_input)        # {!r}   : repr() (타입 확인)
    logger.info("개수: {:d}개", count)              # {:d}   : 정수

  주의:
    logger.info(f"유저: {user_id}")                # (X) 레벨 비활성이어도 f-string 항상 평가
...
def setup_logging() -> None:
    """로깅 시스템 설정. main.py lifespan 에서 1회만 호출."""
    ...
    # 표준 logging 라이브러리와 통합 — uvicorn / sqlalchemy / apscheduler 등 stdlib
    # logging 을 쓰는 라이브러리 로그도 loguru sink 를 통과시킨다.
...
def get_logger(name: str) -> Logger:
    """이름이 지정된 로거 가져오기. scope 는 `<domain>.<context>` (예: `auth.login`)."""

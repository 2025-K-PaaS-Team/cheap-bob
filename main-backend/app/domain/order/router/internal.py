"""order 도메인의 internal API — payment-backend 가 호출.
...
class _CreateOrderHandler:
    ...
    preference 변경 race 는 payment-backend init 시점과 main-backend 의 finalize 시점 사이 customer 가 선호를 바꾸면 갱신된 값이 스냅샷에 들어간다 — 기존 single-process 흐름과 동일.
    """
...
async def create_order_from_cart(
    ...
    """payment-backend 의 finalize 가 호출. payment_id UNIQUE 멱등 — 두 번째 호출은 noop 으로 204."""

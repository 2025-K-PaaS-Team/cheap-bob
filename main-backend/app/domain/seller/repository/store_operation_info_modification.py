...
from app.domain.seller.model.store_operation_info_modification import StoreOperationInfoModification
...
logger = get_logger("seller.repository.store_operation_info_modification")
...
class StoreOperationInfoModificationRepository(BaseRepository[StoreOperationInfoModification]):
    ...
    async def get_all_with_operation_info(self) -> List[StoreOperationInfoModification]:
        """모든 변경 예약을 operation_info 와 함께 조회 (스케줄러 worker 호출)."""
        query = (
            select(StoreOperationInfoModification)
            .options(selectinload(StoreOperationInfoModification.operation_info))
        )
    ...
    async def delete_by_modification_id(self, modification_id: int) -> None:
        """단일 modification 삭제."""
        stmt = (
            delete(StoreOperationInfoModification)
            .where(StoreOperationInfoModification.modification_id == modification_id)
        )

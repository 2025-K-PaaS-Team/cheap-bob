"""seller 도메인의 internal API — payment-backend 가 호출.

shared 패턴 미적용 — payment-backend 의 dto/internal.py 와 일치하는 schema 를 본 모듈에 자체
...
# ───────── schemas (payment-backend 의 dto/internal.py 와 1:1 매칭) ─────────
...
async def consume_stock(
    ...
    """payment-backend 의 결제 init 시 호출. 재고 부족 → 400, 낙관적 락 충돌 → 409.
...
async def restore_stock(
    ...
    """payment-backend 의 보상/취소 시 호출. (payment_id, "restore") PK 로 멱등.
...
async def get_store_id_by_seller_email(
    ...
    """seller_email → store_id. payment-backend 의 seller_settings 라우터가 호출."""
    try:
